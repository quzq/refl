...
#define	USR_SCR_XMIN	(0+32) 
#define	USR_SCR_XMAX	(640-32)
#define	USR_SCR_YMIN	(0+32+32)
#define	USR_SCR_YMAX	(400-32+32)
...
public:
	...
	bool	Move( void )
	{
		...
		/*弾との接触チェック*/
		DownEnergy( msmgr->CheckTouch(GetID(),JYOUKEN_NORMAL,GetX(),GetY(),GetWidth(),GetHeight(),GetPower() ) );
		return( TRUE );
	}
	void	Draw( void )
	{
		...
		/*オフスクリーンサーフェス内の位置情報*/
		int col=0;
		if(GetX()>460) col=64; else if(GetX()>400) col=32;

		rect_from.top = data_rect.top;
		rect_from.bottom = data_rect.bottom;
		rect_from.left = data_rect.left+col;
		rect_from.right = data_rect.right+col;
...
/*													*/
/****************************************************/
class	UsrDefaultBall:public	UsrCharacter	/*UsrCharacterクラスを継承*/
{
private:
	int	addx;		/*Ｘ座標の増分値*/
	int	addy;		/*Ｙ座標の増分値*/
	UsrChrObjectMgr*	objmgr;	/**/
	int	freeze;		
	int	lastcount;
	int	speed;
	int	time;
	int	bect;
	bool	ship_no_touch_old;
	int		freeze_time;/*カーソル判定フリーズ*/
public:		/*コンストラクタ*/
	UsrDefaultBall( int id,int x,int y,int w,int h, int ax, int ay,int sp,UsrChrObjectMgr* mm)
		:UsrCharacter(ATRB_CODE_MISSILE,id,x,y,w,h,1,9999,USR_SCR_XMIN,USR_SCR_XMAX-w,USR_SCR_YMIN,USR_SCR_YMAX-h)
	{
		SetBect( ax, ay );
		objmgr=mm;
		freeze=0;
	...
	bool	Move( void )
	{
		...
		if( GetEnergy()<=0 )
		{
			if( lastcount==30 )
			{
				...
				/*リフレクト機のオブジェクトアドレス取得*/
				/****************************************/
				target=objmgr->GetObject(ID_CHR_GROUP_A,ATRB_CODE_SHIP);
				target->SetEnergy(0);
			}
			if( lastcount++ > 99 )
			{
		...
		/*仮想移動位置の決定*/
		int	i,j;
		int	dmy_x=GetX()*8,dmy_y=GetY()*8,dmg;
		bool	ref_x,ref_y;
		bool	ship_no_touch;
		bool	speedup=FALSE;
		...
		for(i=0;i<speed/2;i++)
		{
			...
			/*レフレクターとの接触*/
			ship_no_touch=CheckRefrecter(dmy_x,dmy_y);
			if( ship_no_touch==FALSE )
			{
				addx=(rand()%3+2)*2;
				if(addy>0)addy=8-addx;
					else addy=addx-8;
				ship_no_touch	=FALSE;
				speedup=TRUE;
				/****************************************/
				...
				target=objmgr->GetObject(ID_CHR_GROUP_A,ATRB_CODE_SHIP);
				dmy_x-=GetX()*8;
				SetX(target->GetX()+target->GetWidth()+1);
				dmy_x+=GetX()*8;
				continue;
			}
			/*進行方向に障害物を調査し、何もなければ(接触物にダメージ)*/
			if( CheckBlock(dmy_x,dmy_y,GetPower())==TRUE && ship_no_touch==TRUE)
			{
				/*進行方向に進む*/
//				SetXY(dmy_x,dmy_y);
				continue;
			...
			/*仮想進行方向決定*/
			/*進行方向に障害物を調査し、何もなければ(接触物にダメージ)*/
			ship_no_touch=CheckRefrecter(dmy_x-addx,dmy_y+addy);
			if( CheckBlock(dmy_x-addx,dmy_y+addy,0)==TRUE && ship_no_touch==TRUE)
			{
				/*進行方向を変更*/
				addx*=-1;
				/*進行方向に進む*/
...
//				SetXY(dmy_x/10,dmy_y/10);
		...
		SetXY(dmy_x/8,dmy_y/8);
		if( speedup==TRUE )speed++;
		return(TRUE);
	}
	bool	CheckBlock(int	x,int	y,int	dmg)
	{
		x/=8;y/=8;
		if( objmgr->CheckTouch(ID_CHR_GROUP_B,JYOUKEN_EQUAL,x,y,GetWidth(),GetHeight(),dmg ) ) return(FALSE);
		return(TRUE);
	}
	bool	CheckRefrecter(int	x,int	y)
	{
		x/=8;y/=8;
		if( objmgr->CheckTouch(ID_CHR_GROUP_A,JYOUKEN_EQUAL,x,y,GetWidth(),GetHeight(),0 ) ) return(FALSE);
		return(TRUE);
	}
	void	Draw( void )
	{
...
private:
	...
	int	OldBtn2;
public:
	UsrShotShip( int id,int x,int y,int w,int h, int b, int s, int p, int e, UsrChrObjectMgr* mm, UsrCtrlDevice* cd )
		:UsrCharacter( ATRB_CODE_SHIP,id,x, y, w, h, p, e,USR_SCR_XMIN+w,USR_SCR_XMAX-w,USR_SCR_YMIN+h,USR_SCR_YMAX-h )
	{		/*コンストラクタ*/
		SetBect(b);
		speed=s;	/*機体の移動スピード*/
		msmgr=mm;
...
public:
	UsrDefaultShip( int id,int x, int y, int w, int h, int b, int s, UsrChrObjectMgr* mm, UsrCtrlDevice* cd )
		:UsrShotShip( id, x, y, w, h, b, s, 100, 100, mm, cd )
	{
		RECT	rect;
		rect.top=0;
		rect.bottom=64;
		rect.left=( ( b==1 ) ? 0:320 );
		rect.right=( ( b==1 ) ? 32:352 );
		SetRect(rect);
	}
	/*	コンストラクタ	*/
...
public:
	UsrTwinShip( int id,int x, int y, int w, int h, int b, int s, UsrChrObjectMgr* mm, UsrCtrlDevice* cd )
		:UsrShotShip( id, x, y, w, h, b, s, 100, 60, mm, cd )
	{
		RECT	rect;
		rect.top=0;
		rect.bottom=64;
...
/********************************************************************/
/*																	*/
/*	クラス名：UsrRefrecterShip										*/
/*																	*/
/********************************************************************/
class	UsrRefrecterShip:public	UsrShotShip	/*UsrCharacterクラスを継承*/
{
private:
public:
	UsrRefrecterShip( int id,int x, int y, int w, int h, int b, int s, UsrChrObjectMgr* mm, UsrCtrlDevice* cd )
		:UsrShotShip( id, x, y, w, h, b, s, 1, 100, mm, cd )
	{
		RECT	rect;
		rect.top=0;
		rect.bottom=64;
...
public:
	SGLib(void)
	{
		...
		UsrShip[USR_PLAYER1]=new	UsrRefrecterShip( ID_CHR_GROUP_A,USR_SCR_XMIN+50, ( USR_SCR_YMAX-USR_SHIP_SIZEY )/2, USR_SHIP_SIZEX/2, USR_SHIP_SIZEY, 1, 8*2, UsrMsMgr, CTRLDev[USR_PLAYER1] );
		...
		for(i=0;i<5;i++)	/*Y方向*/
		{
			for(j=0;j<6;j++)	/*x方向*/
			{
				UsrMsMgr->AddDat(new UsrBlock( ID_CHR_GROUP_B,640-128-j*32,USR_SCR_YMIN+64*i,32,64, 1, UsrMsMgr ));
			}
		}
		...
		UsrMsMgr->AddDat(
		new UsrDefaultBall(	/*接触時にはスプレッド生成*/
			ID_CHR_OTHERS,
			200,
			100,
			32,
