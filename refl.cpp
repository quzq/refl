...
void	BackGroundDraw(LPDIRECTDRAWSURFACE4,int);
/*	グローバル変数宣言	*/
...
static	BOOL	RestoreSurfaces(void)
{
	...
	RECT	rect;
	hBrush = CreateSolidBrush( RGB(0,63,63) );
	hOldBrush = SelectObject(hDCDest, hBrush);
	for(i=0;i<20*15;i+=2)
	{
		rect.top=( i % 15 )*32;
	...
	BackGroundDraw(g_pDDSBackGroundA,1);
...
static	void	DrawTopTitle(HWND	hWnd)
{
	...
	if(g_pDDSBack->GetDC(&hDC) == DD_OK)
	{
		...
		TextOut(hDC,145,384,"Copyright(C)1999 QWERT",strlen("Copyright(C)1999 QWERT"));
		SelectObject(hDC, hFontOld);
		DeleteObject(hFont);
	...
	BackGroundDraw(g_pDDSBack,0);
	//	SpriteDebug();
}
const	char	BgMap[2][15][21]={
/*タイトルＢＧマップデータ*/
//	"00000000000000000000",
//	"00000000000000000000",
//	"00accg0acg0acg0a0000",
//	"00l00t0k000l000j0000",
//	"00k00u0joi0koi0k0000",
//	"00jaoi0l000j000k0000",
//	"00lhog0hoi0h000hoi00",
//	"00j00t00000000000000",
//	"00h00hpqqrnoproooi00",
//	"00000000000000000000",
//	"00000000000000000000",
//	"00000000000000000000",
//	"00000000000000000000",
...
/*タイトルＢＧマップデータ*/
	"00000000000000000000",
	"00000000000000000000",
	"00accgadfgaccga00000",
	"00l00tk000l000j00000",
	"00k00ujooikooik00000",
	"00jaoil000m000k00g00",
	"00mhogj000j000l00t00",
	"00j00thooih000hooi00",
	"00h00hpqqrnoproooi00",
	"00000000000000000000",
	"00000000000000000000",
	"00000000000000000000",
...
/*ステージＢＧマップデータ*/
	"00000000000000000000",
	"acdeeefbcdfcbdfdefcg",
	"k000000000000000000t",
	"l000000000000000000u",
	"m000000000000000000u",
	"m000000000000000000m",
	"j000000000000000000s",
	"k000000000000000000t",
	"m000000000000000000s",
	"l000000000000000000m",
	"l000000000000000000u",
	"k000000000000000000t",
	"hopqqrnopqrnonpqqroi",
	"00000000000000000000",
	"00000000000000000000"
};

void	BackGroundDraw( LPDIRECTDRAWSURFACE4	surface,int stage )
{
	RECT	rect_from,rect_to;/*21*/
	bool	draw_sw;
	int	i,j;
	HDC	hDCDest;
	...
	for(i=0;i<15;i++)
	{
		for(j=0;j<20;j++)
		{
			...
			switch( BgMap[stage][i][j] )
			{
				...
				case	'e':
					rect_from.top	=	32*0;
					rect_from.left	=	32*10;
					break;
				case	'f':
					rect_from.top	=	32*0;
					rect_from.left	=	32*11;
					break;
				case	'g':
					rect_from.top	=	32*0;
					rect_from.left	=	32*12;
					break;
				case	'h':
					rect_from.top	=	32*0;
				...
				case	'i':
					rect_from.top	=	32*0;
					rect_from.left	=	32*14;
					break;
				case	'j':
					rect_from.top	=	32*0;
					rect_from.left	=	32*15;
					break;
				case	'k':
					rect_from.top	=	32*0;
					rect_from.left	=	32*16;
					break;
				case	'l':
					rect_from.top	=	32*0;
					rect_from.left	=	32*17;
					break;
				case	'm':
					rect_from.top	=	32*0;
					rect_from.left	=	32*18;
					break;
				case	'n':
					rect_from.top	=	32*0;
					rect_from.left	=	32*4;
			...
			if( draw_sw==TRUE )
			{
				...
				rect_to.top		=	i*32;
				rect_to.left	=	j*32;
				rect_to.bottom	=	rect_to.top+32;
				rect_to.right	=	rect_to.left+32;
//				surface->Blt( &rect_to, g_pDDSSprite[1], &rect_from,DDBLT_KEYSRC | DDBLT_WAIT,NULL);
				surface->BltFast( rect_to.left, rect_to.top, g_pDDSSprite[1], &rect_from, DDBLTFAST_SRCCOLORKEY |DDBLTFAST_WAIT);
			}
		}
	}
}
